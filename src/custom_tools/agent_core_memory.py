...
tailered actions from 
https://github.com/strands-agents/tools/blob/main/src/strands_tools/memory.py
"""
...
class AgentCoreMemoryToolProvider:
    ...
    def __init__(
        ...
        self.region = region or os.environ.get("AGENTCORE_REGION","us-west-2")
        ...
        memory_control_client = MemoryClient(region_name=self.region)
    ...
    def retrieve_memory(self,
        ...
        Args:
            search_query:  Natural language query to search for finding relevant memories
        ...
        if isinstance(self.namespace,dict):
            ...
            for key in namespace.keys():
                ...
                if isinstance(response, dict):
                    ...
                    relevant_all_data.append({f"Memory type:{key}":relevant_data})
            # logger.info(f"relevant_all_data:{relevant_all_data}")

            return {
                "status": "success",
                "content": [
                    {"text": f"Memories retrieved successfully: {json.dumps(relevant_all_data, default=str,ensure_ascii=False)}"}
                ],
            }
            ...
            return {
                "status": "success",
                "content": [
                    {"text": f"Memories retrieved successfully: {json.dumps(relevant_data, default=str,ensure_ascii=False)}"}
                ],
            }
    ...
    def agent_core_memory(
        ...
        """
        Work with agent memories - search, retrieve, and manage memory records.
        ...
        Args:
            action: The memory operation to perform (one of: "retrieve", "get", "delete")
        ...
        try:
            ...
            try:
                if action_enum == MemoryAction.RETRIEVE:
                    
                    return self.retrieve_memory(search_query=query,
                        max_results=max_results,
                        next_token=next_token,)
    ...
    def delete_all_events(self,session_id:str):
        all_events = self.list_all_events(session_id=session_id)
        logger.info(f"deleting all {len(all_events)} memory events")
        
        for event_id in all_events:
            self.bedrock_agent_core_client.delete_event(
                memoryId = self.memory_id,
                sessionId = session_id,
                eventId = event_id,
                actorId = self.actor_id
            )
        logger.info(f"all memory events are deleted!")
        
    def list_all_events(self,session_id:str):
        all_event_ids = []
        next_token = ""
        while True:
